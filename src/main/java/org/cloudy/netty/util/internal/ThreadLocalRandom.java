 ...
 * tasks (for example, each a {@link org.cloudy.netty.util.internal.chmv8.ForkJoinTask}) use random numbers
...
public final class ThreadLocalRandom extends Random {
    ...
    public static synchronized long getInitialSeedUniquifier() {
        ...
        if (initialSeedUniquifier == 0) {
            ...
            ThreadLocalRandom.initialSeedUniquifier = initialSeedUniquifier =
                    SystemPropertyUtil.getLong("org.cloudy.netty.initialSeedUniquifier", 0);
        }
            ...
            if (seedUniquifier.compareAndSet(current, next)) {
                if (current == 0 && logger.isDebugEnabled()) {
                    logger.debug(String.format(
                            "-Dorg.cloudy.netty.initialSeedUniquifier: 0x%016x (took %d ms)",
