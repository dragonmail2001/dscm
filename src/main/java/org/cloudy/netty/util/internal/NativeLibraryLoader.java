...
public final class NativeLibraryLoader {
    ...
    static {
        ...
        String workdir = SystemPropertyUtil.get("org.cloudy.netty.native.workdir");
        if (workdir != null) {
            ...
            WORKDIR = f;
            logger.debug("-Dorg.cloudy.netty.native.workdir: " + WORKDIR);
        } else {
            WORKDIR = tmpdir();
            logger.debug("-Dorg.cloudy.netty.native.workdir: " + WORKDIR + " (org.cloudy.netty.tmpdir)");
        }
    }
    ...
    private static File tmpdir() {
        ...
        try {
            ...
            if (f != null) {
                logger.debug("-Dorg.cloudy.netty.tmpdir: " + f + " (java.io.tmpdir)");
                return f;
            }
            ...
            if (isWindows()) {
                ...
                if (f != null) {
                    logger.debug("-Dorg.cloudy.netty.tmpdir: " + f + " (%TEMP%)");
                    return f;
                }
                ...
                if (userprofile != null) {
                    ...
                    if (f != null) {
                        logger.debug("-Dorg.cloudy.netty.tmpdir: " + f + " (%USERPROFILE%\\AppData\\Local\\Temp)");
                        return f;
                    }

                    f = toDirectory(userprofile + "\\Local Settings\\Temp");
                    if (f != null) {
                        logger.debug("-Dorg.cloudy.netty.tmpdir: " + f + " (%USERPROFILE%\\Local Settings\\Temp)");
                        return f;
                    }
                }
            } else {
                ...
                if (f != null) {
                    logger.debug("-Dorg.cloudy.netty.tmpdir: " + f + " ($TMPDIR)");
                    return f;
                }
            }
        } catch (Exception ignored) {
