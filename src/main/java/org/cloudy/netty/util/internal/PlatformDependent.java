 ...
 * under the License.
 */
package org.cloudy.netty.util.internal;

import org.cloudy.netty.util.CharsetUtil;
import org.cloudy.netty.util.internal.chmv8.ConcurrentHashMapV8;
import org.cloudy.netty.util.internal.chmv8.LongAdderV8;
 ...
 * the system property <strong>org.cloudy.netty.noUnsafe</strong>.
 */
public final class PlatformDependent {
    ...
    private static final boolean DIRECT_BUFFER_PREFERRED =
            HAS_UNSAFE && !SystemPropertyUtil.getBoolean("org.cloudy.netty.noPreferDirect", false);
    ...
    static {
        if (logger.isDebugEnabled()) {
            logger.debug("-Dorg.cloudy.netty.noPreferDirect: {}", !DIRECT_BUFFER_PREFERRED);
        }
     ...
     * {@code -Dorg.cloudy.netty.noPreferDirect} option.
     */
    ...
    private static boolean hasUnsafe0() {
        boolean noUnsafe = SystemPropertyUtil.getBoolean("org.cloudy.netty.noUnsafe", false);
        logger.debug("-Dorg.cloudy.netty.noUnsafe: {}", noUnsafe);
        ...
        if (noUnsafe) {
            logger.debug("sun.misc.Unsafe: unavailable (org.cloudy.netty.noUnsafe)");
            return false;
        }

        // Legacy properties
        boolean tryUnsafe;
        if (SystemPropertyUtil.contains("org.cloudy.netty.tryUnsafe")) {
            tryUnsafe = SystemPropertyUtil.getBoolean("org.cloudy.netty.tryUnsafe", true);
        } else {
            tryUnsafe = SystemPropertyUtil.getBoolean("org.jboss.netty.tryUnsafe", true);
        }

        if (!tryUnsafe) {
            logger.debug("sun.misc.Unsafe: unavailable (org.cloudy.netty.tryUnsafe/org.jboss.netty.tryUnsafe)");
            return false;
        }
    ...
    private static boolean hasJavassist0() {
        ...
        boolean noJavassist = SystemPropertyUtil.getBoolean("org.cloudy.netty.noJavassist", false);
        logger.debug("-Dorg.cloudy.netty.noJavassist: {}", noJavassist);

        if (noJavassist) {
            logger.debug("Javassist: unavailable (org.cloudy.netty.noJavassist)");
            return false;
        }
    ...
    private static File tmpdir0() {
        File f;
        try {
            f = toDirectory(SystemPropertyUtil.get("org.cloudy.netty.tmpdir"));
            if (f != null) {
                logger.debug("-Dorg.cloudy.netty.tmpdir: {}", f);
                return f;
            }

            f = toDirectory(SystemPropertyUtil.get("java.io.tmpdir"));
            if (f != null) {
                logger.debug("-Dorg.cloudy.netty.tmpdir: {} (java.io.tmpdir)", f);
                return f;
            }
            ...
            if (isWindows()) {
                ...
                if (f != null) {
                    logger.debug("-Dorg.cloudy.netty.tmpdir: {} (%TEMP%)", f);
                    return f;
                }
                ...
                if (userprofile != null) {
                    ...
                    if (f != null) {
                        logger.debug("-Dorg.cloudy.netty.tmpdir: {} (%USERPROFILE%\\AppData\\Local\\Temp)", f);
                        return f;
                    }

                    f = toDirectory(userprofile + "\\Local Settings\\Temp");
                    if (f != null) {
                        logger.debug("-Dorg.cloudy.netty.tmpdir: {} (%USERPROFILE%\\Local Settings\\Temp)", f);
                        return f;
                    }
                }
            } else {
                ...
                if (f != null) {
                    logger.debug("-Dorg.cloudy.netty.tmpdir: {} ($TMPDIR)", f);
                    return f;
                }
            }
        } catch (Throwable ignored) {
    ...
    private static int bitMode0() {
        ...
        int bitMode = SystemPropertyUtil.getInt("org.cloudy.netty.bitMode", 0);
        if (bitMode > 0) {
            logger.debug("-Dorg.cloudy.netty.bitMode: {}", bitMode);
            return bitMode;
        }
        ...
        if (bitMode > 0) {
            logger.debug("-Dorg.cloudy.netty.bitMode: {} (sun.arch.data.model)", bitMode);
            return bitMode;
        }
        bitMode = SystemPropertyUtil.getInt("com.ibm.vm.bitmode", 0);
        if (bitMode > 0) {
            logger.debug("-Dorg.cloudy.netty.bitMode: {} (com.ibm.vm.bitmode)", bitMode);
            return bitMode;
        }
        ...
        if (bitMode > 0) {
            logger.debug("-Dorg.cloudy.netty.bitMode: {} (os.arch: {})", bitMode, arch);
        }
