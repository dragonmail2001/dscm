...
import org.cloudy.netty.util.Recycler;
import org.cloudy.netty.util.Recycler.Handle;
...
import static org.cloudy.netty.util.internal.MathUtil.isOutOfBounds;
import static org.cloudy.netty.util.internal.ObjectUtil.checkNotNull;
import static org.cloudy.netty.util.internal.StringUtil.NEWLINE;
import static org.cloudy.netty.util.internal.StringUtil.isSurrogate;
...
public final class ByteBufUtil {
    ...
    static {
        String allocType = SystemPropertyUtil.get("org.cloudy.netty.allocator.type", "unpooled").toLowerCase(Locale.US).trim();
        ByteBufAllocator alloc;
        if ("unpooled".equals(allocType)) {
            alloc = UnpooledByteBufAllocator.DEFAULT;
            logger.debug("-Dorg.cloudy.netty.allocator.type: {}", allocType);
        } else if ("pooled".equals(allocType)) {
            alloc = PooledByteBufAllocator.DEFAULT;
            logger.debug("-Dorg.cloudy.netty.allocator.type: {}", allocType);
        } else {
            alloc = UnpooledByteBufAllocator.DEFAULT;
            logger.debug("-Dorg.cloudy.netty.allocator.type: unpooled (unknown: {})", allocType);
        }

        DEFAULT_ALLOCATOR = alloc;

        THREAD_LOCAL_BUFFER_SIZE = SystemPropertyUtil.getInt("org.cloudy.netty.threadLocalDirectBufferSize", 64 * 1024);
        logger.debug("-Dorg.cloudy.netty.threadLocalDirectBufferSize: {}", THREAD_LOCAL_BUFFER_SIZE);

        MAX_CHAR_BUFFER_SIZE = SystemPropertyUtil.getInt("org.cloudy.netty.maxThreadLocalCharBufferSize", 16 * 1024);
        logger.debug("-Dorg.cloudy.netty.maxThreadLocalCharBufferSize: {}", MAX_CHAR_BUFFER_SIZE);
    }
