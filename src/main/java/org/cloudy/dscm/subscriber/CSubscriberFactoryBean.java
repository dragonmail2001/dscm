...
public class CSubscriberFactoryBean implements FactoryBean<Object>, InitializingBean {
	...
	private String safe = CConf.SAFE;
		...
		this.proxyObj = Proxy.newProxyInstance(this.getClass().getClassLoader(),
				new Class[] { Class.forName(interfaceName) }, new InvocationHandler() {
					public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
						...
						try{
							return CConnectionExecutorImpl.INST.execute(proxy, method, args, interfaceUrl, sync, safe, uuid);
						}finally{
							CConnectionExecutorImpl.INST.waste(new StringBuffer(interfaceUrl).append("?").
									append(method.getName()).append(" uuid=").append(uuid).append(" all-client-time=").
